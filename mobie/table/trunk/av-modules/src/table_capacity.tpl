...
<!--表头-->
 <div  class="col-xs-12 pa">
    <table class="table table-bordered"  style="margin-bottom: 0;background-color:#0274d4;">
      <col ms-repeat="capahead" ms-css-width="el"/>
     <tr ms-each="capaTitle">
       <td class="cen">{{el}}</td>
     </tr>
</table>
 </div>
...
<!--风情表格数据-->
 <div class="col-xs-12 pa1">
    <table class="table table-bordered" ms-each="capaData"> 
       <col ms-repeat="capahead" ms-css-width="el"/>
       <tr>
         <td class="cen">{{el.stnm}}</td>
         <td class="cen">{{el.allow_water}}</td>
         <td class="cen">{{el.allow_capa}}</td>
         <td class="cen">{{el.allow_rain}}</td>
       </tr>
    </table>
 </div>